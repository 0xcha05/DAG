...
> **📘 For detailed documentation, see [WORKFLOW_ALLOCATION_GUIDE.md](./WORKFLOW_ALLOCATION_GUIDE.md)**
...
**Key Architecture:** Allocation strategy is defined **per KPI** in the config, not in the workflow context.
...
allocationStrategy: {
  ...
  time: {
    year: { strategy: 'historical', historical: { lookbackYears: 2 } }
  }
}
```

**User edits "Electronics, 2024, Sls U = 500,000":**
- Edit level: year → Uses `historical` strategy (seasonal distribution)

**User edits "Electronics, Jan 2024, Sls U = 50,000":**
- Edit level: month → Uses `default` (pro_rata) strategy
...
| **historical** | Based on past patterns (with lookback config) |
| **weighted** | Priority-based (tier/category weights) |
...
// Strategy comes from KPI config (Sls U → default → pro_rata)
```

### Important Notes

1. **Strategy Selection:** Automatic based on edit level (time/hierarchy)
2. **Validation:** Per-KPI configuration for sum validation
3. **Dependent KPIs:** Always recalculate from formulas (never distributed)
4. **Client Flexibility:** Works with any database structure

**See [WORKFLOW_ALLOCATION_GUIDE.md](./WORKFLOW_ALLOCATION_GUIDE.md) for:**
- Detailed strategy selection logic
- Complete SQL generation examples
- Multi-level hierarchy handling
- All allocation strategy details
